 function Move(obj,json,time,M){
		   var startVal={};//存放初始时各属性的值
		   var endVal={};
           ...
           var timer=setInterval(function(){
				...
				for(var key in json){
					var c=endVal[key]-startVal[key];
					var b=startVal[key];
					obj.style[key]=Tween[M](t,b,c,d)+"px";
				}
		   },13);
 }
...
function getElementsByClass(attr){
			if(document.getElementsByClassName){
				...
				for(var i=0;i<obj.length;i++){
					var attrs=obj[i].className.split(" ");
					for(var j=0;j<attrs.length;j++){
						if(attr==attrs[j]){
							result.push(obj[i]);
							break;
